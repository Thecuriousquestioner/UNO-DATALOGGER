...
//++++++++++++++++++++++++++++++++++++++++++++++INITIALIZATION OF THE LCD DIPSLAY
...
void setup() {
   ...
   //the setup is for an LCD screen size of 16 columns and 2 rows.
   //if you LCD screen is different you need to change these values
   //in the LCD.begin command accordingly. 
lcd.begin(16, 2);
lcd.clear();
lcd.setCursor(0,0);
...
//If the file exists, then wait one second a create a new file name.
  while (SD.exists(filename)) {
    ...
    strcat(filename,datename);
    lcd.setCursor(0,0);
    lcd.print("I had to wait an extra second to create the file name:");Serial.println(filename);
  }
lcd.setCursor(0,0);
lcd.print("CARD CHECK OK:");
lcd.setCursor(0,1);
lcd.print(filename);
...
if (!DataLog) {
  lcd.clear();
    lcd.setCursor(0,0);
    lcd.print(F("open failed"));
    return;
  }
  lcd.setCursor(0,0);
  lcd.print(F("FILE OPENED!"));
  //delay(2000);
  //lcd.print(filename);
  //lcd.setCursor(0,1);
  //lcd.print(" was opened succesfully!");
  DataLog.close();
...
DataLog.close();
      ...
      lcd.setCursor(1,1);
      lcd.print("ERROR OPENING THE FILE");
    } 
